...
int main(){
	...
	weightFile.open("Weights.txt", ios::out | ios::trunc);
	debugFile.open("Debug.txt", ios::out | ios::trunc);
	errorFile.open("rmseError.txt", ios::out | ios::trunc);
	errorFile2.open("rmseErrorShort(percent).txt", ios::out | ios::trunc);
	...
	double learningRate = 1.4142135;	//changeable (1 - ...)
	double momentum = 0.25;				//changeable (0 - 0.9) stay below 0.90
	...
	while(epoch < loop){
		...
		//update weights
			...
			// ========= UPDATE WEIGHTS ========= //
				...
				/*
				for(int i = 0; i < Y; i++){		//update hidden bias
